...
class FFT_layer(nn.Module):
    def __init__(self, args):
        ...
        self.conv_update = UNet_original()
        
    def forward(self, source_phase, Fourier_intensity):
        Fourier_intensity = Fourier_intensity.unsqueeze(1)
        
        tensor = torch.stack((source_phase, Fourier_intensity), dim=1)
        tensor = tensor.squeeze()
        source_phase = self.conv_update(tensor)
...
class GSNet_Fourier(nn.Module):
    ...
    def forward(self, source_intensity, image_intensity):
        phase = self.phase_init.unsqueeze(1)
        phase = self.input_layer(phase, image_intensity)
        for layer in self.layers:
            phase = layer(phase, image_intensity)
