...
class Fresnel_layer(nn.Module):
    def __init__(self, args):
        super(Fresnel_layer, self).__init__()
        
        self.phase_updating_network = UNet_original()
        
        self.propagator = Fresnel_propagator(args)
        self.z = args.prop_dist
    ...
    def forward(self, phase, intens_far, intens_near):
        ...
        intens_near = torch.clamp(intens_near, 0.0, 2.0)
        near_amp = torch.sqrt(intens_near)
        near_field = near_amp*torch.exp(phase * 1j)
        
        far_field = self.propagator(near_field, self.z)
        ...
        near_field = self.propagator(far_field, -self.z)
...
class GSNet_Fresnel(nn.Module):
    def __init__(self, phase_init, intensity_init, args, num_layers=10):
        super(GSNet_Fresnel, self).__init__()
        self.layers = nn.ModuleList()
        self.input_layer = Fresnel_layer(args)
        self.phase_init = phase_init
        self.intensity_init = intensity_init
        
        for _ in range(num_layers):
            layer = Fresnel_layer(args)
            self.layers.append(layer)
        
    def forward(self, intens_far):
        
        phase, intens_near = self.input_layer(self.phase_init, intens_far, self.intensity_init)
        for layer in self.layers:
            phase, intens_near = layer(phase, intens_far, intens_near)
