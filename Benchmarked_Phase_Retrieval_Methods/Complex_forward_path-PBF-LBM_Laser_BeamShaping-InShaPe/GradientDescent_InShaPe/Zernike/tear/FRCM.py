...
gt_folder = 'prediction_result/gt_phi'
gt_filenames = sorted(os.listdir(gt_folder))
pred_folder = 'prediction_result/pred_phi'
...
for i in range(len(gt_filenames)):
    ...
    gt_path = os.path.join(gt_folder, gt_filenames[i])
    pred_path = os.path.join(pred_folder, pred_filenames[i])
    print(gt_path)
    print(pred_path)
    ...
    if os.path.exists(pred_path):
        ...
        diff_name = gt_filenames[i][:-4]+"_"+str(round(mae, 3))+".png"
