...
if __name__ == '__main__':
    ...
    plt.imsave('lightsource.png', lightsource.cpu().detach().numpy(), cmap='gray')
    SLM_phase_mask = Phase(near_field)
    plt.imsave('SLMphasemask.png', SLM_phase_mask.cpu().detach().numpy(), cmap='gray')
    ...
    for i, (beamshape, Z, name) in enumerate(train_loader):
        ...
        #'''
        with torch.no_grad():
            machine.SLM_Phase_Mask.zero_()
        #'''
        
        '''
        with torch.no_grad():
            machine.SLM_Phase_Mask.data.copy_(machine.cgh_mask)
        '''
        
        start = time.perf_counter()
        for j in range(iteration):

            far_field = machine(near_field, 0, 'M290 machine Forward simulation')
        ...
        phase = machine.SLM_Phase_Mask.detach()
        phase = Wrap(phase)
