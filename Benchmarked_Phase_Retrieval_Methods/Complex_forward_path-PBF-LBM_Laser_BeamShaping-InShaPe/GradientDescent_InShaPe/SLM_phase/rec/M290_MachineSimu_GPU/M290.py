...
class M290(nn.Module):
    def __init__(self, beamshape, path_lightsource, device, imaging_plane='prefoc'):
        ...
        SLM_Phase_Mask = SLM_Phase_Mask.unsqueeze(0)
        self.cgh_mask = SLM_Phase_Mask
        self.SLM_Phase_Mask = nn.Parameter(torch.zeros_like(SLM_Phase_Mask, dtype=torch.float32))
    ...
    def M290_forward(self, field):
        ...
        field = SubPhase(field, self.SLM_Phase_Mask)
    ...
    def forward(self, field, curvature, simulation, size=None):
        ...
        if(simulation=='M290 machine Forward simulation'):
