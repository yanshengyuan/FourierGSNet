...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = Dataset2channel(args.data, recursive=False, load_data=False,
    ...
    H=0
    W=0
    for i, (I, real, imag) in enumerate(train_loader):
        B, C, H, W = I.shape
        print(H, W)
        break
    if(H!=W):
    ...
    if args.evaluate:
        ...
        acc2 = validate(val_loader, model, criterion, args)
        return
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (I, real, imag) in enumerate(train_loader):
        ...
        I_near = I_near.to(device, non_blocking=True).squeeze()

        # compute output
        output = model(I_near, I_far).squeeze()
...
def validate(val_loader, model, criterion, args):
    def run_validate(loader, base_progress=0):
        ...
        with torch.no_grad():
            ...
            for x, (I, real, imag) in enumerate(loader):
                x = base_progress + x
                ...
                prop=Fresnel_propagator(args)
                if(len(output.shape)<=2):
                    output=output.unsqueeze(0)
                if(len(I_near.shape)<=2):
                    I_near=I_near.unsqueeze(0)
                ...
                for i in range(len(output)):
                    ...
                    '''
                    Idata=I_near[i].squeeze()
                    Ampdata=torch.sqrt(Idata)
                    Neardata=Ampdata*torch.exp(outdata * 1j)
                    Fardata=prop(Neardata, args.prop_dist)
                    Iout=torch.abs(Fardata)**2
                    Iout=Iout.squeeze().cpu()
                    np.save(args.val_vis_path+'/I_pred/npy/'+str(x)+'_'+str(i)+'.npy', Iout)
                    plt.imsave(args.val_vis_path+'/I_pred/img/'+str(x)+'_'+str(i)+'.png' , Iout, cmap='gray')
                    GTdata=I_far[i].squeeze().cpu()
                    np.save(args.val_vis_path+'/I_gt/npy/'+str(x)+'_'+str(i)+'.npy', GTdata)
                    plt.imsave(args.val_vis_path+'/I_gt/img/'+str(x)+'_'+str(i)+'.png' , GTdata, cmap='gray')
                    '''
                    gtdata=Phi[i].squeeze().cpu()
                    outdata=outdata.cpu()
                    np.save(args.val_vis_path+'/Phi_pred/npy/'+str(x)+'_'+str(i)+'.npy', outdata)
                    plt.imsave(args.val_vis_path+'/Phi_pred/img/'+str(x)+'_'+str(i)+'.png' ,outdata, cmap='gray')
                    np.save(args.val_vis_path+'/Phi_gt/npy/'+str(x)+'_'+str(i)+'.npy', gtdata)
                    plt.imsave(args.val_vis_path+'/Phi_gt/img/'+str(x)+'_'+str(i)+'.png' ,gtdata, cmap='gray')
    ...
    l=run_validate(val_loader)/len(val_loader)
