...
if __name__ == '__main__':
    ...
    for i, (beamshape, Z, name) in enumerate(train_loader):
        ...
        for j in range(iteration):
            ...
            loss = poisson_loss(I, beamshape)
            
            machine.zero_grad() # equals to optimizer.zero_grad()
            grad = torch.autograd.grad(loss, machine.zernike_coeffs, create_graph=True)[0].view(-1)
            num_coeffs = grad.shape[0]
            hessian = torch.zeros(num_coeffs, num_coeffs, device=device)

            for k in range(num_coeffs):
                grad_k = torch.autograd.grad(grad[k], machine.zernike_coeffs, retain_graph=True)[0]
                hessian[k] = grad_k.view(-1)
    
            # Damping for stability
            hessian += torch.eye(num_coeffs, device=device) * 1e-3
    
            # Newton-Raphson step
            delta = torch.linalg.solve(hessian, grad)
            
            with torch.no_grad():
                machine.zernike_coeffs -= delta
