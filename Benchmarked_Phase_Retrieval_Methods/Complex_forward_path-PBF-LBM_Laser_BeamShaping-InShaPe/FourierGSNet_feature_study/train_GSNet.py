...
def main_worker(gpu, ngpus_per_node, args):
    ...
    gen_phase_init_vis = gen_phase_init.cpu().numpy()[0]
    gs_phase_init_vis = gs_phase_init.cpu().numpy()[0]
    crop_size = 100
    start_h = (427 - crop_size) // 2
    start_w = (427 - crop_size) // 2
    gen_phase_init_vis = gen_phase_init_vis[start_h:start_h + crop_size, start_w:start_w + crop_size]
    gs_phase_init_vis = gs_phase_init_vis[start_h:start_h + crop_size, start_w:start_w + crop_size]
    plt.imsave('gen_phase_init.png' ,gen_phase_init_vis, cmap='gray')
    plt.imsave('gs_phase_init.png' ,gs_phase_init_vis, cmap='gray')
    
    model = PImodel(gen_phase_init, gs_phase_init, args)
    print(model)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (I, Phi) in enumerate(train_loader):
        ...
        Phi=Phi.squeeze()
        I_near = np.load(args.data + "/lightsource.npy").astype(np.float32)
        I_near = torch.tensor(I_near)
        Phi = Phi.to(device, non_blocking=True)
        I_near = I_near.to(device, non_blocking=True).squeeze()

        # compute output
        output, field = model(I_near, I_far)
        
        field = field.squeeze().detach().cpu().numpy()[0]
        crop_size = 100
        start_h = (427 - crop_size) // 2
        start_w = (427 - crop_size) // 2
        field = field[start_h:start_h + crop_size, start_w:start_w + crop_size]
        print(field.shape)
        plt.imsave('gs_phase.png' , np.angle(field), cmap='gray')
        ...
        with torch.no_grad():
            ...
            for x, (I, Phi) in enumerate(loader):
                ...
                if(num_batch==1):
                    ...
                    output,_ = model(I_near, I_far, epoch, num_batch, args.log_features, output_dir)
                    end = time.perf_counter()
                    inf_time_list.append(end-start)
                if(num_batch!=1):
                    start = time.perf_counter()
                    output,_ = model(I_near, I_far, epoch, num_batch, False, output_dir)
                    end = time.perf_counter()
                    inf_time_list.append(end-start)
                num_batch += 1
