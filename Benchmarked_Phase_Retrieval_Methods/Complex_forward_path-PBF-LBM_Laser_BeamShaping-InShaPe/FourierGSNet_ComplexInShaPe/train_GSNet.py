        ...
        with torch.no_grad():
            ...
            for x, (I, Phi) in enumerate(loader):
                ...
                if(num_batch==1):
                    ...
                    output = model(I_near, I_far, epoch, num_batch, args.log_features, output_dir).squeeze()
                    end = time.perf_counter()
                    inf_time_list.append(end-start)
                if(num_batch!=1):
                    start = time.perf_counter()
                    output = model(I_near, I_far, epoch, num_batch, False, output_dir).squeeze()
                    end = time.perf_counter()
                    inf_time_list.append(end-start)
                num_batch += 1
