...
class Simu_layer(nn.Module):
    ...
    def forward(self, phase, intens_far, intens_near):
        bs_1 = False
        if(intens_far.dim()==2):
            bs_1 = True
        if(bs_1==True):
        ...
        batchsize = intens_far.shape[0]
        intens_near = torch.stack([intens_near] * batchsize, dim=0)
        near_amp = torch.sqrt(intens_near)
        near_field = near_amp*torch.exp(phase * 1j)
        
        far_field, size, curvature = self.machine(near_field, 0, 'M290 machine Forward simulation')
        ...
        far_field = SubIntensity(far_field, intens_far)
        
        near_field = self.machine(far_field, curvature, 'M290 machine Inverse simulation', size)
        ...
        '''
        real_far = real_far.squeeze().detach().cpu().numpy()[0]
        imag_far = imag_far.squeeze().detach().cpu().numpy()[0]
        real_far = normalize(real_far)
        imag_far = normalize(imag_far)
        plt.imshow(real_far, cmap='twilight_shifted')
        plt.axis('off')
        plt.tight_layout(pad=0)
        plt.savefig('real_far.png')
        plt.close()
        plt.imshow(imag_far, cmap='twilight_shifted')
        plt.axis('off')
        plt.tight_layout(pad=0)
        plt.savefig('imag_far.png')
        plt.close()
        '''
...
class GSNet_Simu(nn.Module):
    def __init__(self, args, phase_init, num_layers=10):
        super(GSNet_Simu, self).__init__()
        
        self.machine = M290(args.data+"/lightsource.npy", torch.device('cuda:{}'.format(args.gpu)))
        self.init_phase = phase_init
        self.lightsource = self.machine.lightsource
    ...
    def forward(self, intens_far):
        ...
        phase = self.input_layer(self.init_phase, intens_far, self.lightsource)
        for layer in self.layers:
            phase = layer(phase, intens_far, self.lightsource)
