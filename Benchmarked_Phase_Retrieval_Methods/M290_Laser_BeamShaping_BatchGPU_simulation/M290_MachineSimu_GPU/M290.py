...
class M290(nn.Module):
    def __init__(self, beamshape, path_lightsource, device, imaging_plane='prefoc'):
        ...
        lightsource_npy = np.load(path_lightsource)
    ...
    def M290_forward(self, field, ZernikeCoefficients):
        
        zernike_coefficients = ZernikeCoefficients.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, field, curvature, simulation, ZernikeCoefficients, size=None):
        ...
        if(curvature==0 and size==None and simulation=='M290 machine Forward simulation'):
            field, size, curvature = self.M290_forward(field, ZernikeCoefficients)
