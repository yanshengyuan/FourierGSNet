...
parser.add_argument('--step_size', default=500, type=int,
                    help='step size')
...
class Simple_Forwardpath(nn.Module):
    def __init__(self, HW, defocus_phase_mask):
        super(Simple_Forwardpath, self).__init__()
        
        self.hw = HW
        self.angular_mask = torch.exp(defocus_phase_mask * 1j)
    ...
    def forward(self, source_intensity):
        ...
        nearField = nearField * self.angular_mask
        
        farField = torch.fft.fft2(nearField)
        farField = torch.fft.fftshift(farField)
...
if __name__ == '__main__':
    ...
    val_dataset = get_training_data('../RAF_CDI_defocus')
    
    defocus_kernel = np.load('../RAF_CDI_defocus/Mask_defocus_200.npy')
    ...
    _, first_Phi, _ = next(iter(val_loader))
    HW = first_Phi.shape[-1]

    machine = Simple_Forwardpath(HW, defocus_phase_mask)
    ...
    runtime_list = []
    for i, (I, Phi, lightsource) in enumerate(val_loader):
        ...
        for j in range(iteration):
            ...
            I = I.unsqueeze(0)
            I = I.unsqueeze(0)
            I = f.interpolate(I, size=(762, 762), mode='bilinear', align_corners=False)
            I = I.squeeze()
        ...
        I = I*255
        I = I.cpu().detach().numpy()
        mpimg.imsave(vis_dir+'/pred_I_img/'+str(i)+'_intensity.png', -I, cmap='Greys')
        ...
        mpimg.imsave(vis_dir+'/pred_phi_img/'+str(i)+'_phase.png', phase, cmap='Greys')
