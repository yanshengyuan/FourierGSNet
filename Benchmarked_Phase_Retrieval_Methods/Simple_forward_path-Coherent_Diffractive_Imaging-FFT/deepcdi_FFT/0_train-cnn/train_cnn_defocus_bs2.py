...
trainx_folder = '../RAF_CDI_defocus/train/intensity/npy'
trainy_folder = '../RAF_CDI_defocus/train/phase/npy'
valx_folder = '../RAF_CDI_defocus/test/intensity/npy'
valy_folder = '../RAF_CDI_defocus/test/phase/npy'
...
def main():
  ...
  #--------------------------------------------------
  train_x = np.empty((num_training_samples, 128, 128))
  train_y = np.empty((num_training_samples, 128, 128))
  val_x = np.empty((1000, 128, 128))
  val_y = np.empty((1000, 128, 128))
  ...
  for i in range(num_training_samples):
    ...
    train_x[i] = xarray[317:445, 317:445]
    train_y[i] = yarray
  ...
  for i in range(len(valx_files)):
    ...
    val_x[i] = xarray[317:445, 317:445]
    val_y[i] = yarray
  ...
  history = model.fit(train_x, train_y, shuffle=True, batch_size=2, verbose=1,
    epochs=NUM_EPOCHS, validation_data=(val_x, val_y), callbacks=[lr_scheduler])
  ...
  for i in range(len(valx_files)):
    ...
    test_sample = np.empty((1, 128, 128))
    test_sample[0] = xarray[317:445, 317:445]
    ...
    np.save('deepcdi_CDI_defocus_bs2/Phi_pred/npy/'+str(i)+'_phi_pred.npy' ,pred)
    plt.imsave('deepcdi_CDI_defocus_bs2/Phi_pred/img/'+str(i)+'_phi_pred.png' ,pred, cmap='gray')
    np.save('deepcdi_CDI_defocus_bs2/Phi_gt/npy/'+str(i)+'_phi_gt.npy', yarray)
    plt.imsave('deepcdi_CDI_defocus_bs2/Phi_gt/img/'+str(i)+'_phi_gt.png', yarray, cmap='gray')
